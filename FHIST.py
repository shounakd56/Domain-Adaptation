...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
    ...
    for i in range(args.iters_per_epoch):
       ...
       # x_t, = next(train_target_iter)[:1]
        ...
        target_entropy_weight = 16 * target_entropy_weight / torch.sum(target_entropy_weight)
...
# Convert labels_list into a 16x31 matrix (one-hot encoded labels)
        ...
        loss_im = im(logits_t.view(-1, 6))
        new_mmd=tlmmd(x_s, x_t, labels_matrix, labelt_matrix, kernel_mul=2.0, kernel_num=5, fix_sigma=None)
        loss = cls_loss + transfer_loss * args.trade_off+0.05*loss_im+0.54*mmd+1.4*mcc_loss+0.54*new_mmd
